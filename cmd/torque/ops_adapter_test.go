...
func TestOpsAdapterCapabilitiesJSON(t *testing.T) {
	...
	manifest := findAdapterCapability(result.Adapters, "k8s.manifest.apply")
	if manifest == nil {
		t.Fatalf("missing k8s.manifest.apply in %#v", result.Adapters)
	}
	if manifest.Status != "implemented" || manifest.DiffQuality != "server-side" {
		t.Fatalf("k8s.manifest.apply capability = %#v", manifest)
	}
	if !adapterStringSliceContains(manifest.EvidenceArtifacts, "k8s-manifest-diff.json") ||
	...
	logsCapture := findAdapterCapability(result.Adapters, "k8s.logs.capture")
	if logsCapture == nil {
		t.Fatalf("missing k8s.logs.capture in %#v", result.Adapters)
	}
	if logsCapture.Status != "implemented" || logsCapture.DiffQuality != "bounded-redacted-log-evidence" || logsCapture.Mutating {
		t.Fatalf("k8s.logs.capture capability = %#v", logsCapture)
	}
	if !adapterStringSliceContains(logsCapture.EvidenceArtifacts, "k8s-logs-capture-logs.json") ||
