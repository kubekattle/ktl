...
func newStackCommand(kubeconfig *string, kubeContext *string, logLevel *string, remoteAgent *string) *cobra.Command {
	...
	cmd.AddCommand(newStackPlanCommand(&rootDir, &profile, &clusters, &output))
	cmd.AddCommand(newStackApplyCommand(&rootDir, &profile, &clusters, &output, &planOnly, kubeconfig, kubeContext, logLevel, remoteAgent))
	cmd.AddCommand(newStackDeleteCommand(&rootDir, &profile, &clusters, &output, &planOnly, kubeconfig, kubeContext, logLevel, remoteAgent))
	return cmd
}

func newStackPlanCommand(rootDir, profile *string, clusters *[]string, output *string) *cobra.Command {
