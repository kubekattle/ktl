...
func runBuildLogin(cmd *cobra.Command, opts loginOptions) error {
	...
	cfg, err := loadDockerConfigFile(opts.AuthFile, cmd.ErrOrStderr())
	if err != nil {
		return err
	}
	...
	if err := ensureDockerConfigDir(cfg.Filename); err != nil {
		return err
	}
...
func runBuildLogout(cmd *cobra.Command, server string, authFile string) error {
	...
	cfg, err := loadDockerConfigFile(authFile, cmd.ErrOrStderr())
	if err != nil {
		return err
	}
