...
func newDriftCommand(kubeconfig *string, kubeContext *string) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "drift",
		Short: "Observe workload drift over time",
	}
	cmd.AddCommand(newDriftWatchCommand(kubeconfig, kubeContext))
	return cmd
}

func newDriftWatchCommand(kubeconfig *string, kubeContext *string) *cobra.Command {
