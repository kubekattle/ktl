...
usage() {
  cat <<'EOF'
Usage: scripts/e2e/ops/OPS-TR-001.sh [options]
...
OPS-TR-001 proves the SSH transport primitive against a real lab.ssh-linux
host. It connects, runs a command, uploads and downloads a temp file, records a
bounded timeout, redacts command/output evidence, removes remote temp state, and
exports a standard ops evidence bundle.
...
[[ "${TORQUE_OPS_E2E_CONFIRM:-}" == "1" ]] || ops_fail "refusing lab.ssh-linux transport E2E without TORQUE_OPS_E2E_CONFIRM=1"
ops_require_env TORQUE_LAB_SSH
ops_require_cmd go
ops_require_cmd ssh
ops_require_cmd scp

ops_init_run "OPS-TR-001"
scratch_root="$(mktemp -d "${TMPDIR:-/tmp}/torque-ops-tr-001.XXXXXX")"
started_at="$(ops_utc_now)"
ssh_remote_root="/tmp/torque-ops-tr-001-${OPS_RUN_ID}"
...
proof_json="${OPS_RUN_DIR}/transport/ssh-transport-proof.json"
go_test_json="${OPS_RUN_DIR}/go/sshtransport-test.jsonl"

{
  printf 'authorization: bearer %s\n' "${OPS_SECRET_CANARY}"
  printf 'token=%s\n' "${OPS_SECRET_CANARY}"
  printf 'secret://ops/tr-001#token\n'
} | ops_redact_stdin "${OPS_RUN_DIR}/transport/redaction-probe.txt"

ops_log "run SSH transport package tests"
if go test -json ./internal/ops/transport/ssh >"${go_test_json}"; then
  package_test_status="succeeded"
else
  package_test_status="failed"
fi

ops_log "run lab.ssh-linux SSH transport proof"
if TORQUE_OPS_TR_E2E_TARGET="${TORQUE_LAB_SSH}" \
  TORQUE_OPS_TR_E2E_OUTPUT="${proof_json}" \
  TORQUE_OPS_TR_E2E_REMOTE_ROOT="${ssh_remote_root}" \
  TORQUE_OPS_TR_E2E_CANARY="${OPS_SECRET_CANARY}" \
  go test ./internal/ops/transport/ssh -run TestE2EEnvOpenSSHTransport -count=1 >"${OPS_RUN_DIR}/go/e2e-ssh-transport.out" 2>&1; then
  transport_status="succeeded"
else
  transport_status="failed"
fi

ops_log "verify SSH transport proof redaction and operation receipts"
...
if proof_path.is_file():
    ...
    errors.append("missing SSH transport proof")
...
doc = {
    ...
    "kind": "OpsSSHTransportRedactionCheck",
...
if package_test_status != "succeeded":
    errors.append("SSH transport package tests failed")
if transport_status != "succeeded":
    errors.append("SSH transport E2E failed")
for name in ["connect", "prepare", "upload", "copy", "download"]:
    if operations.get(name, {}).get("status") != "succeeded":
...
write_json(
    run_dir / "decision.json",
    {
        ...
        "taskId": task_id,
        "runId": run_id,
        "reason": "ssh-transport-real-lab-proof",
