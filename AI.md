...
### 2.7 Deploy Planning
- Use `ktl deploy plan --chart <path> --release <name> --namespace <ns> --kubeconfig ~/.kube/archimedes.yaml [-f values.yaml]` to render Helm manifests and diff them against the live cluster before mutating anything.
...
- Highlight the creates/updates/deletes plus any pod-affecting changes (Deployments/StatefulSets/Jobs) and PodDisruptionBudget removals so humans understand disruption risk before running `ktl deploy apply`.
