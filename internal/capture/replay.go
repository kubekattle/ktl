...
// ReplayEntries streams capture entries from the given artifact to emit, applying
// the same filtering and limiting rules as Replay. Returning io.EOF from emit
// stops the replay cleanly.
func ReplayEntries(ctx context.Context, artifactPath string, opts ReplayOptions, emit func(Entry) error) error {
	if emit == nil {
	...
	wrapped := func(entry Entry) error {
		if entry.FormattedTimestamp == "" && !entry.Timestamp.IsZero() {
			entry.FormattedTimestamp = entry.Timestamp.Format(time.RFC3339Nano)
		}
	...
	if useJSON {
		return replayFromJSON(ctx, jsonPath, wrapped, opts.Follow)
	}
	return replayFromSQLite(ctx, sqlitePath, wrapped, opts)
}
...
func Replay(ctx context.Context, artifactPath string, opts ReplayOptions, out io.Writer) error {
	...
	return ReplayEntries(ctx, artifactPath, opts, func(entry Entry) error {
