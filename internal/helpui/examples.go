...
var curatedExamples = map[string][]string{
	"torque": {
		...
		"# Start a durable JetStream NATS assignment worker with identity receipts\ntorque-agent nats worker --nats-url nats://127.0.0.1:4222 --delivery jetstream --ledger-path ./.torque/agent/assignments.sqlite --subject torque.assign.lab.host_mysql-01 --agent-id agent-mysql-01 --tenant lab --target-id host/mysql-01 --capability host.command.run",
