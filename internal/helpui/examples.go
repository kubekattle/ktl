...
var curatedExamples = map[string][]string{
	"torque": {
		...
		"# Start a NATS assignment worker with identity receipts\ntorque-agent nats worker --nats-url nats://127.0.0.1:4222 --subject torque.lab.assign.mysql --agent-id agent-mysql-01 --tenant lab --target-id host/mysql-01 --queue mysql-workers --capability host.command.run",
