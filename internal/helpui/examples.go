...
var curatedExamples = map[string][]string{
	...
	"torque stack plan": {
		...
		"# Plan a host.service.manage node with unit state evidence\ntorque stack plan --config ./stacks/host-service --output json",
	...
	"torque stack apply": {
		...
		"# Start and enable a host service with before/after unit receipts\ntorque stack apply --config ./stacks/host-service --yes",
