...
var curatedExamples = map[string][]string{
	"torque": {
		...
		"# Start a verified JetStream NATS assignment worker\ntorque-agent nats worker --nats-url nats://127.0.0.1:4222 --delivery jetstream --ledger-path ./.torque/agent/assignments.sqlite --subject torque.assign.lab.host_mysql-01 --agent-id agent-mysql-01 --worker-id agent-mysql-01-a --tenant lab --target-id host/mysql-01 --queue mysql-target-pool --capability host.command.run --verify-assignments --trusted-issuer-key ./assignment-pub.json --policy-digest sha256:approved-policy --max-deliver 3 --ack-wait 30s --nak-delay 1s",
		"# Report discovered local agent capabilities\ntorque-agent capabilities report --format json",
		"# Publish and inspect NATS agent heartbeats with discovered capabilities\ntorque-agent nats heartbeat --nats-url nats://127.0.0.1:4222 --tenant lab --agent-id host-141 --label role=mysql --worker-slots 2",
		...
		"# Prove a 5-node PostgreSQL plus PgBouncer stack over NATS in Firecracker VMs\nTORQUE_OPS_E2E_CONFIRM=1 scripts/e2e/ops/STACK-FC-POSTGRES-001.sh --evidence-root /tmp/torque-ops-e2e --cleanup",
	...
	"torque ops agent status": {
		...
		"# Read compact status from etcd instead of live NATS\ntorque ops agent status --source store --store etcd --etcd-endpoints http://127.0.0.1:2379 --tenant lab",
	},
	"torque ops agent registry": {
		"# Compact durable heartbeat events into the registry\ntorque ops agent registry compact --nats-url nats://127.0.0.1:4222 --store etcd --etcd-endpoints http://127.0.0.1:2379 --tenant lab",
	},
	"torque ops agent registry compact": {
		"# Run a one-shot compaction from JetStream into etcd\ntorque ops agent registry compact --nats-url nats://127.0.0.1:4222 --tenant lab --store etcd --etcd-endpoints http://127.0.0.1:2379 --max-messages 100",
		"# Compact into a local file registry for development\ntorque ops agent registry compact --store file --store-path ./.torque/ops/agent-registry.json --max-messages 100",
	},
	...
	"torque stack plan": {
		...
		"# Plan the NATS-dispatched Firecracker Kafka lab\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_KAFKA_NATS_LAB_SUBJECT=torque.lab.kafka torque stack plan --config ./testdata/stack/e2e/26-firecracker-kafka-nats-cluster --output json",
		"# Plan the NATS-dispatched Firecracker RabbitMQ lab\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_RABBITMQ_NATS_LAB_SUBJECT=torque.lab.rabbitmq torque stack plan --config ./testdata/stack/e2e/27-firecracker-rabbitmq-nats-cluster --output json",
	...
	"torque stack apply": {
		...
		"# Apply the NATS-dispatched Firecracker Kafka lab with traffic generation\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_KAFKA_NATS_LAB_SUBJECT=torque.lab.kafka torque stack apply --config ./testdata/stack/e2e/26-firecracker-kafka-nats-cluster --yes",
		"# Apply the NATS-dispatched Firecracker RabbitMQ lab with traffic generation\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_RABBITMQ_NATS_LAB_SUBJECT=torque.lab.rabbitmq torque stack apply --config ./testdata/stack/e2e/27-firecracker-rabbitmq-nats-cluster --yes",
	...
	"torque stack delete": {
		...
		"# Delete the NATS-dispatched Firecracker Kafka lab\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_KAFKA_NATS_LAB_SUBJECT=torque.lab.kafka torque stack delete --config ./testdata/stack/e2e/26-firecracker-kafka-nats-cluster --yes",
		"# Delete the NATS-dispatched Firecracker RabbitMQ lab\nTORQUE_NATS_URL=nats://127.0.0.1:4222 TORQUE_RABBITMQ_NATS_LAB_SUBJECT=torque.lab.rabbitmq torque stack delete --config ./testdata/stack/e2e/27-firecracker-rabbitmq-nats-cluster --yes",
	},
	"torque stack status": {
