...
var curatedExamples = map[string][]string{
	"torque": {
		...
		"# Start a verified JetStream NATS assignment worker\ntorque-agent nats worker --nats-url nats://127.0.0.1:4222 --delivery jetstream --ledger-path ./.torque/agent/assignments.sqlite --subject torque.assign.lab.host_mysql-01 --agent-id agent-mysql-01 --tenant lab --target-id host/mysql-01 --capability host.command.run --verify-assignments --trusted-issuer-key ./assignment-pub.json --policy-digest sha256:approved-policy --max-deliver 3 --ack-wait 30s --nak-delay 1s",
