...
Reference manual for every HTML-based `ktl` surface (current `ktl logs --ui`, `ktl build --ui`, `ktl apply --ui`, `ktl delete --ui`, and `ktl plan --format=html --visualize`). Keep this doc open while designing or implementing UI changes, and update it first when the system evolves.
...
5. Have you run the relevant visual tests (manual `ktl logs --ui` / `ktl build --ui` / `ktl apply --ui` / `ktl delete --ui`, screenshot diffs) and noted them in your PR?
